...
export const POST = auth(async (req) => {
  try {
    ...
    const svgs = body.svgs || {};
    const svgKeys = Object.keys(svgs).filter((k) => svgs[k] && String(svgs[k]).trim().length > 0);
    if (process.env.NODE_ENV === "development") {
      console.log("[FIGMA-IMPORT] Payload:", { name: body.name, nodesRootCount: body.nodes?.length, svgsReceived: svgKeys.length, svgIds: svgKeys.slice(0, 10) });
    }
