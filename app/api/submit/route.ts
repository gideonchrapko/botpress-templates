...
export async function POST(req: NextRequest) {
  try {
    ...
    // Regular template handling (default primaryColor for Figma-only templates that don't have a color field)
    const primaryColor = (formData.get("primaryColor") as string)?.trim() || "#3D9DFF";
